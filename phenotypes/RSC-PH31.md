...
This template phenotype identifies prescription or administration events for a specified drug or vaccine from longitudinal primary care records using a SNOMED CT codelist.  The phenotype returns a list of events per patient. Applied across populations, it supports estimation of prescribing rates or vaccine coverage, cohort construction, and derivation of patient-level variables for downstream analysis.
...
| `drug_codelist`      | SNOMED CT codes identifying prescription or administration events for the specific drug or vaccine |
...
* **Type:** events
* **Description:**
  A list of prescription/administration events **WHAT IS RETURNED?DATE?INITIATING EVENT?** for the specified drug or vaccine within the observation window.

## Pseudocode

* Let `candidate_events` be all events in `patient_record` with codes from `drug_codelist` within `observation_window`.
* For each event in `candidate_events`:
  * The event is considered to define a separate event if:
    * same_day_filter is false, or there are no prior events from `drug_codelist` on the same day as the event.
